...
"""
Training script for the decoder-only transformer image captioning model.
Based on adam_baseline_training_v2.py — only the model import, instantiation,
log file name, and default paths/lr have been changed.

To run locally:
  python decoder_transformer_training.py --epochs 30 --checkpoint False --dataset_dir flickr8k
      --log_dir runs/decoder_transformer_pass_1 --save_path saved_models/decoder_transformer_pass_1/

To run on cluster:
  Use a corresponding .sh script (same structure as transformer_model_local.sh)
"""
...
def main(opt):
    ...
    # CHANGED: decoder-only transformer model instantiation
    P = 16
    embed_dim = 256
    num_heads = 8
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr", type=float, default=1e-4, help="starting learning rate")  # CHANGED: 0.001 -> 1e-4
    ...
    parser.add_argument("--log_dir", type=str, default="./runs/decoder_transformer_pass_1", help="tensorboard log directory")  # CHANGED
    parser.add_argument("--save_path", type=str, default="./saved_models/decoder_transformer_pass_1/", help="directory to save checkpoints")  # CHANGED
