...
class Flickr8kDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Caption format:
            ...
            2. Pad or truncate to max_caption_length using <PAD>
        ...
        # Step 2: pad to max_caption_length (truncate if too long)
        pad_index = self.vocab.word_to_index[Vocabulary.PAD_TOKEN]
        if len(token_ids) < self.max_caption_length:
            # Append <PAD> tokens until we reach max_caption_length
            token_ids = token_ids + [pad_index] * (self.max_caption_length - len(token_ids))
        else:
            # Truncate captions longer than max_caption_length
            token_ids = token_ids[:self.max_caption_length]

        # Step 3: wrap with <SOS> at start and <EOS> at end
        token_ids = (
