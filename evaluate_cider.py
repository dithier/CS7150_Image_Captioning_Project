...
Usage:
    python evaluate_cider.py --checkpoint_path saved_models/models_3_14/model.pt
...
def compute_doc_freq(crefs, n):
    ...
    for refs in crefs:
        # unique ngrams across all references for this image
        all_ngrams = set()
        for ref in refs:
...
def compute_cider_n(test_counts, refs_counts, doc_freq, num_images, n, sigma=6.0):
    ...
    # average length of references (for Gaussian penalty)
    ...
    # Gaussian length penalty (sigma=6.0, same as COCO toolkit)
    penalty = math.exp(
...
def compute_cider(ref_dict, hypotheses_dict, n_max=4, sigma=6.0):
    ...
    # Pre-cook: build ngram count dicts for all refs and hyps
    # refs_ngrams[n][i] = list of count-dicts (one per reference caption)
    # test_ngrams[n][i] = count-dict for hypothesis
    ...
    # corpus mean, scaled by 10 (COCO toolkit convention)
